...
## 16. Открытые вопросы / риски

- **Публичность** `ContinentRegions` и `WorldAccessBlocker` — не найдены в публичном
  списке репозиториев; проверить перед добавлением ссылок (иначе ведут в 404).
- **Личный Telegram** в `content.ts` (`kfcbossalbino`) — оставить, убрать или заменить
  на группу? По умолчанию: группа `@denfry_dev` как основной CTA.
- **Концепт воксельной формы** (во что собираются блоки: монограмма / изо-куб /
  мини-сцена) — финализируем на этапе реализации Hero.
- **Домен** — оставить `*.vercel.app` или подключить кастомный?
